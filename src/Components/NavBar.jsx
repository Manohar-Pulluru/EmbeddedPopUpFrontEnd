...
export const NavBar = ({ activeIndex, setActiveIndex }) => {
  ...
  const options = [
    ...
    // { name: "Message", iconName: icons.message },
  ];
  ...
  const topPosition = topOffset + activeIndex * 136; // 34 * 4px = 136px
  const mobileItemWidth = 64;
  ...
  return (
    <div className="h-full w-full flex sm:flex-col flex-row bg-[#1F1D2B] relative">
      ...
      {isMobile && (
        <div
          className="absolute sm:hidden bottom-0 h-16 w-16 transition-all duration-300 ease-in-out z-0"
          style={{ left: `${leftPosition}px` }}
        >
          <div className="w-full h-2 bg-[#1F1D2B] rounded-tl-3xl"></div>
          <div className="w-full flex-1 bg-[#1F1D2B] flex items-start justify-center">
            <div className="w-16 h-16 bg-[#EA7C69] rounded-2xl mt-[-12px]"></div>
          </div>
        </div>
      )}
      ...
      {options.map((option, index) => {
        ...
        return (
          <div
            key={index}
            className="w-16 sm:w-full flex items-center justify-center sm:h-34 h-16 cursor-pointer z-10 relative"
            onClick={() => setActiveIndex(index)}
          >
            <div className="w-full sm:ml-2 flex items-center justify-center">
              <Icon
      ...
      {!localStorage.getItem("aftoAuthToken")?.length ? (
        ...
        <div onClick={triggerLogout} className="sm:h-34 cursor-pointer flex-col sm:w-full absolute bottom-0 aspect-square sm:flex hidden items-center justify-center z-10">
          <svg
            width="32"
            height="32"
            viewBox="0 0 24 24"
